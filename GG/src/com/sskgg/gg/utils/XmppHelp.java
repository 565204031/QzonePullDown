    ...
    private static XMPPConnection getConnection()
    {
    	if(connection==null)
    	{
    		final ConnectionConfiguration connectionConfig = new ConnectionConfiguration(
    	            "192.168.5.100", 5222, "user-20150314el");
...
//                reg.getPacketID()), new PacketTypeFilter(IQ.class));  
        ...
        PacketCollector collector = getConnection()  
        ...
        if (result == null) {  
            LogUtils.i("RegistActivity", "No response from server.");  
            return "服务器没有结果";  
        } else if (result.getType() == IQ.Type.RESULT) {  
            return "注册成功";  
        } else { // if (result.getType() == IQ.Type.ERROR)  
            if (result.getError().toString().equalsIgnoreCase("conflict(409)")) {  
            	LogUtils.i("RegistActivity", "IQ.Type.ERROR: "  
    ...
    public static List<RosterEntry> searchAllFriends() {  
        ...
        Collection<RosterEntry> roscol = getConnection().getRoster()  
                .getEntries(); 
    ...
    public static List<RosterGroup> searchAllGroup() {  
        ...
        Collection<RosterGroup> roscol = getConnection().getRoster()  
                .getGroups();  
    ...
    public static List<RosterEntry> getGroupFriends(String group) {  
        List<RosterEntry> entries = new ArrayList<RosterEntry>();  
        RosterGroup rosgrou = getConnection().getRoster()  
                .getGroup(group);  
    ...
    public static boolean addNewGroup(String group) {  
        try {  
            getConnection().getRoster().createGroup(group);  
            return true;  
        } catch (Exception e) {  
            e.printStackTrace();  
            return false;  
...
//    }  
            ...
            public void chatCreated(Chat chat, boolean able) {  
                chat.addMessageListener(new MessageListener() {  
                    ...
                    public void processMessage(Chat chat2, Message message) {  
                        LogUtils.i("connectMethod:getOnLine()", message.getBody()  
                                + message.getFrom());  
    ...
    public static void sendTalkMsg(String to, String msg) {  
        Chat chat = getConnection().getChatManager()  
                .createChat(to, null);  
        try {  
