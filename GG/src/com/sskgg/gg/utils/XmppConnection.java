...
import com.sskgg.gg.listener.TaxiConnectionListener;
import com.sskgg.gg.listener.onlineMultiUserMessageListener;
import com.sskgg.gg.listener.onlineMultiUserStateListener;
...
public class XmppConnection {
    ...
    private String SERVER_HOST = "192.168.5.100";  
    private XMPPConnection connection = null;  
    private String SERVER_NAME = "user-20150314el";  
    ...
    public void closeConnection() {  
        ...
        LogUtils.i("XmppConnection", "關閉連接");  
    }  
    ...
    public String regist(String account, String password) {  
        ...
        collector.cancel();  
        if (result == null) {  
            LogUtils.i("regist", "No response from server.");  
            return "0";  
        } else if (result.getType() == IQ.Type.RESULT) {  
        	LogUtils.i("regist", "regist success.");  
            return "1";  
        } else { // if (result.getType() == IQ.Type.ERROR)  
            if (result.getError().toString().equalsIgnoreCase("conflict(409)")) {  
            	LogUtils.i("regist", "IQ.Type.ERROR: "  
    ...
    public void setPresence(int code) {  
        ...
        case 0:  
            ...
            LogUtils.i("state", "设置在线");  
            break;  
        case 1:  
            presence = new Presence(Presence.Type.available);  
            presence.setMode(Presence.Mode.chat);  
            con.sendPacket(presence);  
            LogUtils.i("state", "设置Q我吧");  
            break;  
        case 2:  
            presence = new Presence(Presence.Type.available);  
            presence.setMode(Presence.Mode.dnd);  
            con.sendPacket(presence);  
            LogUtils.i("state", "设置忙碌");  
            break;  
        case 3:  
            presence = new Presence(Presence.Type.available);  
            presence.setMode(Presence.Mode.away);  
            con.sendPacket(presence);  
            LogUtils.i("state", "设置离开");  
            break;  
        case 4:  
            ...
            for (RosterEntry entry : entries) {  
                ...
                con.sendPacket(presence);  
                LogUtils.i("state", presence.toXML());  
            }  
            // 向同一用户的其他客户端发送隐身状态  
            presence = new Presence(Presence.Type.unavailable);  
            presence.setPacketID(Packet.ID_NOT_AVAILABLE);  
            ...
            con.sendPacket(presence);  
            LogUtils.i("state", "设置隐身");  
            break;  
        case 5:  
            presence = new Presence(Presence.Type.unavailable);  
            con.sendPacket(presence);  
            LogUtils.i("state", "设置离线");  
            break;  
        default:  
            break;  
        }  
...
//    }  
    ...
    public boolean addGroup(String groupName) {  
        ...
        try {  
            ...
            LogUtils.i("addGroup", groupName + "創建成功");  
            return true;  
        } catch (Exception e) {  
            e.printStackTrace();  
            return false;  
	...
	public Chat getFriendChat(String friend, MessageListener listenter) {  
	    ...
	    // 创建聊天窗口 
	    Chat chat = XmppConnection.getInstance().getConnection().getChatManager().createChat(friend + "@"+  
	    		XmppConnection.getInstance().getConnection().getServiceName(), listenter);  
	    // 添加聊天窗口到chatManage   
    ...
    public List<HostedRoom> getHostRooms() {  
        ...
        try {  
            ...
            LogUtils.i("room", "服务会议数量:" + roominfos.size());  
        } catch (XMPPException e) {  
            e.printStackTrace();  
        }  
        return roominfos;  
    ...
    public MultiUserChat createRoom(String user, String roomName,  
        ...
        try {  
            ...
            // 登录房间对话  
            submitForm.setAnswer("muc#roomconfig_enableLogUtilsging", true);  
            // 仅允许注册的昵称登录  
     ...
     * 会议室对象
     */
    private Map<String, MultiUserChat> MultiUserChatManage = new HashMap<String, MultiUserChat>();// 聊天窗口管理map集合  
    /** 
     * 加入会议室 
     *  
     * @param user 
     ...
     *            会议室名 
     */  
    public MultiUserChat joinMultiUserChat(Context ct,String user, String roomsName,  
            String password) {  
        if (getConnection() == null)  
            return null;  
        try {  
        	MultiUserChat muc=MultiUserChatManage.get(roomsName);
        	if(muc==null)
        	{
        		//使用XMPPConnection创建一个MultiUserChat窗口  
                muc = new MultiUserChat(getConnection(), roomsName);  
                // 聊天室服务将会决定要接受的历史记录数量  
                ...
                //添加监听消息
                muc.addMessageListener(new onlineMultiUserMessageListener(ct,roomsName)); 
                //监听会议室状态（成员的进入、离开等）
                muc.addParticipantStatusListener(new onlineMultiUserStateListener(ct,roomsName));
                LogUtils.i("MultiUserChat", "会议室【"+roomsName+"】加入成功........");  
        	}
            return muc;  
        } catch (XMPPException e) {  
            e.printStackTrace();  
            LogUtils.i("MultiUserChat", "会议室【"+roomsName+"】加入失败........");  
            return null;  
        }  
    } 
	   /**
	   ...
	   *  @return * 
	   */  
   	public List<MultiUserList> getConferenceRoom() throws XMPPException { 
   		...
   		MultiUserList friendrooms = new MultiUserList(); 
   		friendrooms.setRoom(j.getName());//聊天室的名称 
   		//friendrooms.setJid(j.getJid());//聊天室JID 
   		LogUtils.i("INFO", j.getName()+"|"+j.getJid());
